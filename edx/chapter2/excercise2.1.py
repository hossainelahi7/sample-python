...
def get_threshold(unique_values, gini_scores):
    ...
    if idx < len(unique_values) - 1:
        ...
        threshold = unique_values[idx]  # If it's the last value, use it directly
    return threshold
...
# Create a new DataFrame where all x2 values are less than or equal to x2_threshold
...
# Subset first_split_df to include only points in the left child of the 2nd split for x1
